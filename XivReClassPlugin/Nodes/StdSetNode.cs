...
public class StdSetNode : BaseWrapperArrayNode {
	...
	protected override Size DrawChild(DrawContext context, int x, int y)
	{
		...
		nint GetLeft(nint node) => context.Process.ReadRemoteIntPtr(node);
		nint GetParent(nint node) => context.Process.ReadRemoteIntPtr(node + 0x08);
		nint GetRight(nint node) => context.Process.ReadRemoteIntPtr(node + 0x10);
		bool IsNil(nint node) => context.Process.ReadRemoteUInt8(node + 0x19) == 1;
